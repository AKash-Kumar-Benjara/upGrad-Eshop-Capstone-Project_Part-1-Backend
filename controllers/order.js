...
async function createOrder(req, res) {
    ...
    if(!product) {
        return res.status(400).send({message:`No Product found for ID - ${req.body.product}!`});
    }

    const address = await Address.findById(req.body.address);
    if(!address) {
        return res.status(400).send({message:`No address found for ID - ${req.body.address}!`});
    }

    if(product.availableItems < req.body.quantity) {
        return res.status(400).send({message:`Product with ID - ${req.body.product} is currently out of stock!`});
    }
